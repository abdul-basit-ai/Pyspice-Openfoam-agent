...
def draw_schematic(
    ...
    if topology == "buck":
        # Vin -> HS switch -> sw node -> L -> out; LS switch sw->gnd; Cout; Rload
        ...
        d += elm.Dot()
        out_node = d.here
        d += elm.Switch().down(d.unit * 1.5).label(f"LS\n{mosfet_pn}")
        ...
        d += elm.Dot()
        out_node = d.here
        d += (cout := elm.Capacitor().down(d.unit * 1.2).label(f"{capacitor_pn}\n{capacitor_uf:.1f}µF"))
        d += elm.Ground().at(d.here)
        d += elm.Line().right(d.unit).at(out_node)
        d += elm.Resistor().down(d.unit * 1.2).label(f"Load\n{vout}V")
        d += elm.Ground().at(d.here)
    else:  # buck_boost (4-switch non-inverting, Phase 3's topology)
        d += elm.SourceV().up().label(f"Vin\n{vin}V")
        d += elm.Line().up(d.unit / 2)
